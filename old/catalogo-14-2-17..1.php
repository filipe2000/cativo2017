<!DOCTYPE html>
 <html class="no-js" lang="pt-br"> 
   
    <head>
    <title>Cativo Comercial e Logística</title>	
    <?php include 'meta.php' ?>
    </head>
<body>
<?php include "header.php" ?> 
<div class="container">		
	
	<div class="row subR ">						
	<div id="cat" class="col-md-9 col-md-offset-1 col-sm-10 col-sm-offset-1 col-xs-12 ">		  
         <a href="catalogo/catalogoCativo.pdf" target="blank"> <h1 style="text-align: center;">
        <br><br><br><i class="fa fa-newspaper-o" aria-hidden="true"></i>
        Cat&aacute;logo.</h1></a><br><br><br><br><br><br><br>
        				<br><br><br><br><br><br><br>
        				<br><br><br><br><br><br>
         
		<!-- <a href="includes/catalogo/catalogo-10-16.pdf" target="blank"> <h1>
		<br><br><br><i class="fa fa-newspaper-o" aria-hidden="true"></i>
		Cat&aacute;logo.</h1></a><br><br><br><br><br><br><br>


		 <script>
		 $(window).load(function(){
		  $('#cat')
		    .html('<iframe src="catalogo/cat.html" class="flipPdf"></iframe>');
		});
		 </script>
		 -->
		</div>
	</div>
	
</div>					
</body>   
<section id="foot" class="bg-f">
	<?php include ('footer.php');  ?>			
</section>
<?php include "metaJS.php";  ?>
</html>